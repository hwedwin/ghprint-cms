 ...
 */
@Controller
public class QueryUserlistController extends BaseAction{
    private static Logger logger = LoggerFactory.getLogger(QueryUserlistController.class);

    @Autowired
    private UserService userService;

    @RequestMapping(value = "/userinfo/queryuserlist.do", method = RequestMethod.GET)
    @ResponseBody
    public DataGridResult getUserlist(HttpServletRequest request, HttpServletResponse response, TSysUser user, Integer page, Integer rows){
        ...
        try {
            ...
            logger.info("queryuserlist request Param:{}", user);
            if (flag) {
                dataGridResult = userService.selectByParam(user, page, rows);
    ...
    public String getAuthorityId() {
        return AuthorityKey.USER_QUERY;
    }
}
